    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public double GetArea()
        {
            double area = 2 * Math.PI * this.Radius * this.Radius + 
                          2 * Math.PI * this.Radius * (this.Top - this.Bottom).Magnitude;
            return area;
        }
        ...
        {
            double volume = Math.PI * this.Radius * this.Radius * (this.Top- this.Bottom).Magnitude;
            return volume;
        }
    ...
    public class AdvancedFigureController : FigureController
    {
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        IAreaMeasurable currFigureAdvanced = this.currentFigure as IAreaMeasurable;
                        if (currFigureAdvanced != null)
                        {
                            Console.WriteLine("{0:0.00}", currFigureAdvanced.GetArea());
                        }
                        else
                        {
                ...
                case "normal":
                    {
                        IFlat currFigureAdvanced = this.currentFigure as IFlat;
                        if (currFigureAdvanced != null)
                        {
                            Console.WriteLine("{0:0.00}", currFigureAdvanced.GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "volume":
                    {
                        IVolumeMeasurable currFigureAdvanced = this.currentFigure as IVolumeMeasurable;
                        if (currFigureAdvanced != null)
                        {
                            Console.WriteLine("{0:0.00}", currFigureAdvanced.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
...
namespace AcademyGeometry
{
    class Program
    {
        private static AdvancedFigureController GetFigureController()
        {
