...
namespace CohesionAndCoupling
{
    static class GeometryUtils2D
    {
        ...
        {
            double distance = GeometryUtils2D.CalcDistance2D(0, 0, width, height);
            return distance;
        }

        public static double CalcDiagonalXZ(double width, double depth)
        {
            double distance = GeometryUtils2D.CalcDistance2D(0, 0, width, depth);
            return distance;
        }

        public static double CalcDiagonalYZ(double height, double depth)
        {
            double distance = GeometryUtils2D.CalcDistance2D(0, 0, height, depth);
            return distance;
        }
    }
}
